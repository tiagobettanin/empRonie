Aqui está a explicação completa da resolução do seu simulado de Processamento Digital de Sinais (PDS), preenchendo as lacunas e detalhando o raciocínio por trás de cada questão.
...
* **Definição do Sistema:** Pela imagem, vemos que a resposta ao impulso é:
    $h[n] = 1 \cdot \delta[n] - 1 \cdot \delta[n-1] + 1 \cdot \delta[n-2]$
...
1.  **Cálculo de $H(\omega)$:**
    A DTFT é calculada pela soma $H(\omega) = \sum h[n]e^{-j\omega n}$.
...
2.  **Manipulação para Fase Linear:**
    ...
    $H(\omega) = e^{-j\omega}(e^{j\omega} - 1 + e^{-j\omega})$
    Usando a identidade de Euler ($2\cos(\omega) = e^{j\omega} + e^{-j\omega}$), agrupamos os termos:
    $H(\omega) = e^{-j\omega}((e^{j\omega} + e^{-j\omega}) - 1)$
    $H(\omega) = e^{-j\omega}(2\cos(\omega) - 1)$

3.  **Módulo $|H(\omega)|$:**
    O módulo é o valor absoluto da amplitude:
    $|H(\omega)| = |e^{-j\omega}| \cdot |2\cos(\omega) - 1| = \mathbf{|2\cos(\omega) - 1|}$
    * Em $\omega=0$: $|2\cos(0) - 1| = |2-1| = 1$.
    * Em $\omega=\pi/3$: $|2\cos(\pi/3) - 1| = |2(1/2) - 1| = 0$. (Zero do filtro)
    * Em $\omega=\pi/2$: $|2\cos(\pi/2) - 1| = |0-1| = 1$.
    * Em $\omega=\pi$: $|2\cos(\pi) - 1| = |2(-1) - 1| = |-3| = 3$.
...
Para uma entrada $x[n]=\cos(\omega_0 n)$, a saída $y[n]$ é $A \cos(\omega_0 n + \varphi)$, onde $A = |H(\omega_0)|$ e $\varphi = \angle H(\omega_0)$. Basta lermos os valores dos gráficos:

* **$\omega_0 = 0$:**
    * $A = |H(0)| = \mathbf{1}$
    * $\varphi = \angle H(0) = \mathbf{0}$
* **$\omega_0 = \pi/4$:**
    * $A = |H(\pi/4)| = |2\cos(\pi/4) - 1| = |2(\sqrt{2}/2) - 1| = \mathbf{\sqrt{2} - 1}$ (aprox. 0.414)
    * $\varphi = \angle H(\pi/4) = -\pi/4$ (está na região onde a fase é $-\omega$)
* **$\omega_0 = \pi/2$:**
    * $A = |H(\pi/2)| = \mathbf{1}$
    * $\varphi = \angle H(\pi/2)$. Aqui, $\omega=\pi/2$, que está fora da faixa central. A fase é $-\omega + \pi = -\pi/2 + \pi = \mathbf{\pi/2}$.
* **$\omega_0 = \pi$:**
    * $A = |H(\pi)| = \mathbf{3}$
    * $\varphi = \angle H(\pi)$. Está fora da faixa central. A fase é $-\omega + \pi = -\pi + \pi = \mathbf{0}$.
...
* Entrada: $x[n] = (\frac{1}{2})^n u[n] \implies X(z) = \frac{1}{1 - \frac{1}{2}z^{-1}}$ , com Região de Convergência (ROC) $|z| > 1/2$.
* Saída: $y[n] = (\frac{1}{4})^n u[n-1]$.
    * Reescrevemos $y[n]$ para usar a propriedade do atraso: $y[n] = \frac{1}{4} (\frac{1}{4})^{n-1} u[n-1]$.
    * $Y(z) = \frac{1}{4} \left( \frac{z^{-1}}{1 - \frac{1}{4}z^{-1}} \right)$, com ROC $|z| > 1/4$.

### (a) Polos, Zeros e ROC de H(z)

A função de transferência $H(z)$ é a razão entre as transformadas:

$H(z) = \frac{Y(z)}{X(z)} = \frac{\frac{1}{4}z^{-1} / (1 - \frac{1}{4}z^{-1})}{1 / (1 - \frac{1}{2}z^{-1})}$
$H(z) = \frac{\frac{1}{4}z^{-1}(1 - \frac{1}{2}z^{-1})}{1 - \frac{1}{4}z^{-1}} = \frac{\frac{1}{4}z^{-1} - \frac{1}{8}z^{-2}}{1 - \frac{1}{4}z^{-1}}$
...
O rascunho da divisão na página 5 está confuso e leva a um resultado incorreto.
...
Esta é a parte mais direta. Começamos com a $H(z)$ e cruzamos os denominadores:
$\frac{Y(z)}{X(z)} = \frac{\frac{1}{4}z^{-1} - \frac{1}{8}z^{-2}}{1 - \frac{1}{4}z^{-1}}$
$Y(z) \cdot (1 - \frac{1}{4}z^{-1}) = X(z) \cdot (\frac{1}{4}z^{-1} - \frac{1}{8}z^{-2})$
...
Isolando $y[n]$:
$y[n] = \frac{1}{4}y[n-1] + \frac{1}{4}x[n-1] - \frac{1}{8}x[n-2]$
O rascunho na página 6 está correto.

---

## 🧐 Questão 3: Verdadeiro ou Falso

* **(a) Falso.** "É possível determinar unicamente $h[n]$ se forem conhecidas as localizações de polos e zeros... e o valor de $H(z_0)$..."
    * **Justificativa:** Está FALSO. Faltou a informação mais crucial: a **Região de Convergência (ROC)**. Polos e zeros idênticos podem corresponder a um sistema causal, um anti-causal ou um de dois lados, cada um com um $h[n]$ diferente. A ROC define qual deles é.
* **(b) Verdadeiro.** "Se um sistema é FIR, então $H(z)$ não possui polos, exceto possivelmente em $z=0$."
    * **Justificativa:** Está VERDADEIRO. Um sistema FIR causal tem $H(z) = \sum_{n=0}^{M} h[n]z^{-n}$. Ao colocar em uma fração (potências positivas), $H(z) = \frac{h[0]z^M + ... + h[M]}{z^M}$. O denominador é $z^M$, o que significa que todos os $M$ polos estão na origem ($z=0$).
* **(c) Falso.** "Se $H(z)$ tem mais polos do que zeros, o sistema é causal."
    * **Justificativa:** Está FALSO. A causalidade não tem relação com a *quantidade* de polos ou zeros. Ela é definida pela ROC (exterior ao polo mais externo) ou pela condição $h[n]=0$ para $n<0$.
* **(d) Falso.** "Se $H(z)$ tem todos os seus polos e zeros dentro do círculo unitário, o sistema é estável."
    * **JustificTiva:** Está FALSO. A estabilidade depende da ROC incluir o círculo unitário ($|z|=1$). Se um sistema for *anti-causal* (ROC é *interna* ao polo mais interno), ter todos os polos *dentro* do círculo (ex: $p=0.5$) faria com que a ROC ($|z|<0.5$) *não* incluísse o círculo unitário, tornando-o instável. (Os zeros são irrelevantes para a estabilidade).

---

## 🎶 Questão 4: Convolução via Transformada Z

Devemos calcular $y[n] = x[n] * h[n]$, onde:
...
1.  **Transformada $X(z)$:**
    $x[n] = -(-\frac{1}{2}) \cdot (-\frac{1}{2})^{n-1} u[n-1] = \frac{1}{2} \cdot (-\frac{1}{2})^{n-1} u[n-1]$
    Usando a propriedade $a^{n-1}u[n-1] \leftrightarrow \frac{z^{-1}}{1-az^{-1}}$:
    $X(z) = \frac{1}{2} \left( \frac{z^{-1}}{1 - (-\frac{1}{2})z^{-1}} \right) = \mathbf{\frac{1}{2} \frac{z^{-1}}{1 + \frac{1}{2}z^{-1}}}$

2.  **Transformada $H(z)$:**
    $h[n] = \delta[n] + (\frac{1}{2})^{n-1} u[n-1]$
    $H(z) = \mathbf{1 + \frac{z^{-1}}{1 - \frac{1}{2}z^{-1}}}$
...
3.  **Multiplicação $Y(z)$:**
    ...
    $H(z) = \frac{(1 - \frac{1}{2}z^{-1}) + z^{-1}}{1 - \frac{1}{2}z^{-1}} = \frac{1 + \frac{1}{2}z^{-1}}{1 - \frac{1}{2}z^{-1}}$
    ...
    $Y(z) = \mathbf{\frac{1}{2} \frac{z^{-1}}{1 - \frac{1}{2}z^{-1}}}$
...
4.  **Transformada Inversa $y[n]$:**
    ...
    A resolução nas páginas 6 e 7 está correta.
...
* **Resposta Correta: D**
* **Justificativa:** A DTFT $X(\omega) = \sum x[n]e^{-j\omega n}$ é uma soma infinita. Uma condição *suficiente* para que ela convirja (exista) é que a sequência $x[n]$ seja **absolutamente somável** (ou seja, $\sum |x[n]| < \infty$). Além disso, a função $e^{-j\omega n}$ é inerentemente periódica em $\omega$ com período $2\pi$, o que torna $X(\omega)$ também **periódica com período $2\pi$**.

### Questão 6: ROC e Estabilidade

* **Resposta Correta: E**
* **Justificativa:** A estabilidade de um sistema LTI (LTI) requer que $h[n]$ seja absolutamente somável ($\sum |h[n]| < \infty$). A DTFT $H(\omega)$ é a Transformada Z $H(z)$ avaliada em $z=e^{j\omega}$ (o círculo unitário). A DTFT só existe se $h[n]$ for absolutamente somável (estável). Portanto, a condição para estabilidade é que a DTFT exista, o que significa que a **ROC de $H(z)$ deve incluir o círculo unitário ($|z|=1$)**.
...
* **Justificativa:** O Teorema de Nyquist-Shannon afirma que para reconstruir perfeitamente um sinal analógico (limitado em banda $F_{max}$) a partir de suas amostras, a frequência de amostragem $F_s$ deve ser pelo menos o dobro da frequência máxima.
    * Condição: **$F_s \ge 2F_{max}$**.
    * Violação: Se $F_s < 2F_{max}$ (subamostragem), as réplicas espectrais no domínio da frequência se sobrepõem, um fenômeno destrutivo chamado **aliasing**, que impossibilita a recuperação do sinal original.
...
* **Justificativa:** O processo de amostragem cria réplicas do espectro original em múltiplos da frequência de amostragem ($\Omega_s$). O filtro de reconstrução ideal (um filtro passa-baixas ideal) tem duas funções:
    ...
    2.  **Preservar a banda base** original (de $0$ a $\Omega_{max}$), aplicando um **ganho de T** (período de amostragem) para compensar a atenuação de $1/T$ que ocorre durante o processo de amostragem por impulsos.

---

Espero que esta explicação detalhada ajude a esclarecer todos os pontos do simulado!

Você gostaria que eu detalhasse algum dos conceitos, como a divisão longa de polinômios em $z^{-1}$ ou a relação entre a fase e os termos $\cos(\omega)$?
